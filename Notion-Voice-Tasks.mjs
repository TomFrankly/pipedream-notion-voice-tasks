...
import validate from "./helpers/validate.mjs"; // Validate user input
import cost from "./helpers/cost.mjs"; // Calculate cost
const config = {
	notion_dbs: {
		tasks: {
			id: "",
...
export default {
			...
			async options({ query, prevContext }) {
				if (this.notion) {
					try {
						...
						let sortedDbs = response.results.sort((a, b) => {
							const aTitle = a.title?.[0]?.plain_text.toLowerCase() || '';
							const bTitle = b.title?.[0]?.plain_text.toLowerCase() || '';
							
							if (aTitle === 'tasks' || aTitle.includes('tasks')) return -1;
							if (bTitle === 'tasks' || bTitle.includes('tasks')) return 1;
							return 0;
						  });
	...
	async additionalProps() {
		...
		// Check for presence of Priority properties
		const priorityFlag = properties.hasOwnProperty("Priority") && properties["Priority"].type === "select" ? true : false;
		...
		function buildOptions(props, topOption) {
			const uniqueProps = props.filter((value, index, self) => self.indexOf(value) === index);
			const options = topOption && uniqueProps.includes(topOption)
				? [topOption, ...uniqueProps.filter(prop => prop !== topOption)]
				: uniqueProps;
			return options.map((prop) => ({ label: prop, value: prop }));
		}
		...
		const props = {
			taskName: {
				...
				description: "Select the title property for your tasks. There will be only one option here, which is the title property. If you're using the Ultimate Brain or Ultimate Tasks templates, the option should be either \"Name\" or \"Task\".",
			...
			status: {
				...
				options: buildOptions(combinedProps, "Status"),
				optional: true,
				reloadProps: true,
			},
			source: {
			...
			...(this.status && {
				status_value: {
					...
					description:
						"Choose a value for your chosen Status property.",
					options: this.status
						? properties[this.status][
							properties[this.status]?.type
					  ].options.map((option) => ({
								label: option.name,
								value: option.name,
						  }))
	...
	methods: {
		...
		setPropChoices() {
			...
			config.properties = {
				...
				...(this.status_value && 
					{ tasks_status_value: this.status_value }
				),
				...(this.priority && {
				...
				properties: {
					...
					...(config.properties.tasks_status_value && {
						[config.properties.tasks_status.name]: {
							[config.properties.tasks_status.type]: {
								// Can be Status or Select
				...
				children: [
					{
						...
						callout: {
							...
							rich_text: [
								{
									text: {
										content: `This task was created via the following source: ${source}.`,
									},
								},
							],
						},
