...
export default defineComponent({
	props: {
		...
		secretKey: {
			...
			label: "Secret Key",
			description:
				"Set a secret key here that matches the secret key from your iOS/Android shortcut exactly. When you workflow receives a new request, the secret key in the request body will be compared against this value. If they match, the workflow will continue. This prevents others from sending requests to your workflow, even in the rare event that they knew your request URL.",
			reloadProps: true,
		},
	},
		...
		const props = {
			...(this.openai && {
				chat_model: {
					type: "string",
					label: "ChatGPT Model",
					description: `Select the model you would like to use. Defaults to ${
						currentDate < switchDate ? "gpt-3.5-turbo-0613" : "gpt-3.5-turbo"
					}, which is recommended for this workflow.`,
					default:
						currentDate < switchDate ? "gpt-3.5-turbo-0613" : "gpt-3.5-turbo",
			...
			...(this.openai && {
				update_system: {
					...
					description:
						"Set to True if you want the your workflow to fetch the latest versions of the system messages (instructions for ChatGPT). This setting allows for using updated system messages in the event that better ones are discovered or bugs are discovered in the hard-coded ones (without you having to recreate the entire workflow). You can read the system messages here: https://thomasjfrank.com/mothership-pipedream-notion-voice-tasks/ – if this is set to False, or if the request to that URL fails/takes more than 2 seconds, the script will fall back to the system message that are hard-coded into this workflow. Defaults to True.",
					optional: true,
					default: true,
				},
			}),
